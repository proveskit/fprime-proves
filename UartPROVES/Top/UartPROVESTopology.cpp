// ======================================================================
// \title  UartPROVESTopology.cpp
// \brief cpp file containing the topology instantiation code
//
// ======================================================================
// Provides access to autocoded functions
#include <UartPROVES/Top/UartPROVESTopologyAc.hpp>
#include <UartPROVES/Top/UartPROVESPacketsAc.hpp>
...
void configureTopology() {
    ...
    // tlmSend.setPacketList(UartPROVESPacketsPkts, UartPROVESPacketsIgnore, 1);
...
// Public functions for use in main program are namespaced with deployment name UartPROVES
...
void setupTopology(const TopologyState& state) {
    ...
    if (state.uartDevice != nullptr) {
        ...
        if (hubComDriver.open(state.uartDevice, static_cast<Drv::LinuxUartDriver::UartBaudRate>(state.baudRate), 
                           Drv::LinuxUartDriver::NO_FLOW, Drv::LinuxUartDriver::PARITY_NONE, Svc::DeframerCfg::RING_BUFFER_SIZE)) {
            hubComDriver.start(COMM_PRIORITY, Default::STACK_SIZE);
        } else {
            printf("Failed to open UART device %s at baud rate %" PRIu32 "\n", state.uartDevice, state.baudRate);
        }
    }
